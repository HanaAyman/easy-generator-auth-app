...
describe('Auth flow (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    // Important: import only after the environment variables exist.
  ...
  it('signs in and fetches the protected profile with the session cookie', async () => {
    ...
    const sessionCookie = signInRes.headers['set-cookie'] as string[];
  ...
  it('logs out and clears the session cookie', async () => {
    ...
    expect(clearedCookies[0]).toMatch(/access_token=;/);
  });
});
